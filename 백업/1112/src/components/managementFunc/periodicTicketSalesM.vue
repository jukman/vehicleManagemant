            ...
            <dl>
              ...
              <dd>
                <select v-model="r.product_name">                  
                  <option v-for="(parking_product, index) in parking_product_list" :key="index" :value="r.product_name">
                    {{ parking_product.parking_product_name }}
                  </option>
                </select>
                {{r.product_name}}{{parking_product_list.parking_product_name}} 
              </dd>              
            </dl>
            <dl>
              <dt>그룹</dt>
...
export default {
  ...
  methods: {
    modifyOpen(c) {
      ...
      this.m.str_sales_time = format_time1(new Date(c.sales_time))
  ...
  created() {
    this.$socket.on("message_from_server", data => {
      if(data.kind === "get_sales_manager_list"){
        console.log("아하",this.parking_product_list);
