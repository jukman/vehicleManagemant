...
<div class="report_wrap">
    <div class="title">
      <h2>{{mainTitle[2].subMenu[2].title}}</h2>
    </div>
 ...
 // components: { spinners },
  methods: {
    ...
    //  console.log("socket_process===>", data)
      ...
      else if (data.status === "ok" && data.kind ===  "get_resident_discounted_list_admin_out") {  
        ...
        if(this.searchResidentID === "" || this.searchResidentID ==="전체") {   
            ...
            if(obj.obj_in && obj.obj_in.discounted_list){
              for(let j = 0; j < obj.obj_in.discounted_list.length; j++){          
                if (obj.kind === "lpr" || obj.kind == "fee_calculation_result") {
                  ...
                  obj1.discounted_time = format_time2(new Date(obj.obj_in.discounted_list[j].discounted_time))
                  ...
                  let test = isNaN(obj.obj_in.discounted_list[j].free_parking_time)
                  // console.log("isNaN(obj.discounted_list[j].free_parking_time) : ",isNaN(obj.discounted_list[j].free_parking_time))
                  if(test){
                    
                    obj1.free_parking_time = (obj.obj_in.discounted_list[j].free_parking_time)/(1000*60)
                  }
                  else{
                    obj1.free_parking_time = 60
                  }
                  obj1.discounted_duration = obj1.free_parking_time + ", " + (obj.obj_in.discounted_list[j].discounted_duration)/(1000*60)
                  obj1.resident_id  = obj.obj_in.discounted_list[j].id
                  let test1 = isNaN(obj.obj_in.discounted_list[j].resident_fee)
                  // console.log("isNaN(obj.discounted_list[j].free_parking_time) : ",isNaN(obj.discounted_list[j].free_parking_time))
                  if(test1){
                    
                    obj1.resident_fee = obj.obj_in.discounted_list[j].resident_fee
                  }
                  else{
                    obj1.resident_fee = 0
                  }
          ...
          for (let i = 0; i < vehicle_obj_list.length; i++) {
            ...
            for(let j = 0; j <  obj.obj_in.discounted_list.length; j++){                        
              if (obj.kind === "lpr" || obj.kind == "fee_calculation_result") {              
                if(obj.obj_in && obj.obj_in.discounted_list){
                  if(obj.obj_in.discounted_list[j].id === this.searchResidentID){
                    let obj1 = {}
                    obj1.out_time = format_time1(new Date(obj.loop_event_time))
                    obj1.in_time = format_time1(new Date(obj.obj_in.loop_event_time))
                    obj1.discounted_time = format_time1(new Date(obj.obj_in.discounted_list[j].discounted_time))
                    obj1.loop_event_time = obj.loop_event_time
                    obj1.location = obj.location
                    obj1.direction = obj.direction
                    obj1.lp = obj.lp
