    ...
    <div class="title">
      ...
      <form>                  
          ...
          <dl>
            ...
            <dd><span>1순위</span>
              <select v-model="procedure.type" id="input_id">
                <option v-for="items in procedure" :value="items.order" :key="items.value" >{{items.type}}</option>              
              </select>
            </dd>              
            <dd><span>2순위</span>
              <select id="input_id">
            ...
            <dt>할인순서 &amp; 요금관리{{procedure.value}}
              <button @click.prevent="openPop4(); modifyVehicle1();" class="modifybtn">
             <font-awesome-icon icon="tools" />수정
              </button>
              </dt>            
...
export default {
  data() {
    return {
      ...
      procedure: [
        {value:"1",order:"first",type:"min"},
        {value:"2",order:"second",type:"won"},
        {value:"3",order:"third",type:"percent"}            
      ],
      price_manage: {
        service_time: "10",
        grace_time: "0",
  ...
  methods: {
    ...
    openPop4(){
      ...
      this.modal=true
      console.log("왜",this.procedure.type);
    ...
    changeText(){
      if(this.procedure[0].type ==="min"){
        this.procedure[0].type="할인시간"
        this.procedure[1].type="할인요금"
        this.procedure[2].type="할인률"      
        }
    },    
    modifyOpen(c) {
      this.modifyIs = true
